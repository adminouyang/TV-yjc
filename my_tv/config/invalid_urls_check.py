...
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(threadName)s - %(message)s', handlers=[logging.FileHandler("my_tv/config/error_urls_log", "w", encoding="utf-8"), logging.StreamHandler()])
...
if __name__ == "__main__":
    with open("my_tv/config/error_urls", 'r', encoding='utf-8') as f:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers = 512) as executor:
            ...
            with open("my_tv/config/error_urls_host_log", "w", encoding="utf-8") as error_urlsf:
